...
- **Workflow:** Appropriate — view-only for the Sales Manager. Only Marketing Specialist can create or edit listings and upload photos or 360-views, and only the CEO can approve a price or permanently delete. The Sales Manager is not asked to publish, price, or archive, which keeps the view simple. *(Updated 10 August 2026: pending price proposals are no longer CEO-only in code — Head Accountant sees them read-only on this page since migration `00039`; approval remains CEO-exclusive.)*
...
- **Workflow:** Appropriate and well-scoped — Sales Manager can create and update acquisition, delivery and sourcing cases and assign mechanics; recovery case creation stays with Head Accountant and CEO. Informants and mechanics are selectable as workers. This keeps Sales as coordinator, not just observer. One operational caveat (10 August 2026 audit): the standalone Create dialog always fails server validation because it submits neither a transaction nor a vehicle link ("A transaction or vehicle is required." — see Module audit §3.17), so case creation currently succeeds only where another flow supplies that link; the Sales Manager's own DB write grants are intact.
...
- All recommendations are suggestions only — no functionality has been removed. Controls such as Marketing Specialist-only vehicle create/edit and media upload, CEO-only price approval and vehicle deletion (with Head Accountant holding read-only proposal visibility per migration 00039), Mechanic-only inspection checklist editing, buy-now inquiry filter for Sales Manager, CEO-only roadmap write, field-case kind rules (acquisition and delivery for CEO, Confidential Informant, and Sales Manager; sourcing for CEO and Sales Manager; recovery reserved for CEO and Head Accountant — noting the field-cases RLS write grants currently cover ceo/sales_manager plus head_accountant INSERT, so the Confidential Informant's documented write paths are DB-blocked today), mechanic-assignment limited to CEO, Confidential Informant, and Sales Manager, staff-review management limited to CEO and Account Manager, and CEO-only announcement publish and expiry should be preserved when adding summaries or filters.
