...
> **Status (recorded 10 August 2026):** Superseded in part by the RBAC/task31 pass. This report's central recommendation — a dedicated supplier portal replacing the shared Customer set — was **implemented**: `ROLE_NAV_ACCESS.supplier` is now `{supplier-overview, supplier-messages}` with landing `/supplier/overview` (`src/lib/auth/roles.ts:173,268`), a Supplier Portal sidebar group exists, `src/app/(supplier)/overview/page.tsx` renders own profile/documents/verification progress/messages, and old `/supplier/showroom…` URLs redirect back to the overview (locked by `src/lib/auth/roles.supplier.test.ts` and `src/tests/e2e/supplier-portal.spec.ts`). Sections below that describe the shared seven-module portal are **historical**; onboarding-chain observations remain accurate.

## Summary

At audit time the Supplier had access to 7 modules — all under Customer Portal, identical to the Customer: Showroom, Find Your Car, My Inquiries, Transactions, Request a Car, Sell Vehicle, and Favourites. Compared to the CEO (23 staff modules) or Account Manager (19), this is a narrow, shopping-focused set. That set is correct for a Customer, but not for a Supplier. *(Historical: the RBAC/task31 pass replaced this set with the dedicated two-item supplier portal described in the status banner above.)*
...
Sidebar order at audit time was Customer Portal (Showroom → Find Your Car → My Inquiries → Transactions → Request a Car → Sell Vehicle → Favourites). The configured landing page after sign-in was the showroom browsing page, which is browsing-first — correct for a Customer, but not for a Supplier waiting on approval or documents. *(Today: the sidebar shows the Supplier Portal group — My Supplier Profile → Messages — and sign-in lands on `/supplier/overview`.)*
...
- Supplier Messages is built into the Supplier Portal group of the sidebar (and reachable from the overview page), so it is discoverable without typing an address. *(Updated 10 August 2026; at audit time it had no navigation entry.)*
...
- **Keep as is:** None of the 7 Customer Portal modules needed to be kept as the Supplier's primary workspace. The only staff tool that should be kept for Supplier is Supplier Messages — which is now discoverable in the Supplier Portal sidebar group as implemented by the task31 pass.
