...
- **Recommendation:** Keep — keep view-only access. Consider showing a finance-focused summary on top (price, acquisition cost, status) because the full marketing inventory view is broader than finance needs. No edit access needed. *(Corrected 10 August 2026: pending price proposals are no longer CEO-only — migration `00039` gave Head Accountant read-only visibility of the pending-proposal list on the vehicles page, with no approve buttons; CEO remains the sole approver, and only the vehicle-detail proposal panel stays CEO-exclusive.)*
...
- **Workflow:** Appropriate — strong separation of duties: Account Manager and CEO prepare runs and enter compensation; Head Accountant and CEO review; only Head Accountant can finalize. This prevents the preparer from approving their own work. **Known limitation (10 August 2026 RBAC audit):** these review/finalize steps are enforced at the action layer only — the `payroll_runs` FOR ALL policy in `00017_phase6_payroll.sql` grants writes to ceo/account_manager, not head_accountant, so the DB currently blocks the documented HA transitions until an HA UPDATE-policy migration lands.
...
- **Recommendation:** Keep — keep full detail and the exclusive mark-paid control. Preserve paid_at and paid_by audit fields. **Known limitation (10 August 2026 RBAC audit):** mark-paid/finalize UPDATEs pass the action guards but are denied by the `payslips` RLS grant (00017, ceo/account_manager only) until a migration adds the HA policy; adding payslip items is preparer-only (ceo/account_manager) by design, so HA never adds items.
...
- **Consider for Revision/Removal:** No module is recommended for full removal. The previously suggested addition — a read-only view of pending vehicle price proposals for Head Accountant — has been implemented (migration `00039`: read-only list visibility on the vehicles page, no approval power). All suggestions are non-destructive and preserve existing approvals, verification steps, and audit trails — no functionality should be deleted.
...
- Overly broad or missing: Vehicles is broader than finance needs today — the full marketing inventory is shown when only price and cost context is needed; a summary would reduce clutter. Attendance at full log depth is operationally heavy without a summary. No major module is missing — the former CEO-only price-proposal gap is closed by migration `00039` (read-only list view). Direct access to Staff Records compensation detail is also not needed because compensation is already surfaced inside Payroll; that separation is correct. One precision note: Head Accountant may upload supplier documents at the action/storage level (`uploadSupplierDocument` allows ceo/account_manager/head_accountant) but has no Suppliers page or nav entry, so that capability is only exercisable on behalf of another workflow.
