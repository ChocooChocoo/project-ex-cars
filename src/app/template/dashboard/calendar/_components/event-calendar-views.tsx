...
const getNormalDayHeaderBorderClass = (info: DayHeaderInfo) =>
  cn(!info.inPopover && (info.isMajor ? "border border-foreground/20" : !info.isNarrow && "border"));
const getMutedDayHeaderBorderClass = (info: DayHeaderInfo) =>
  cn(!info.inPopover && (info.isMajor ? "border border-foreground/20" : !info.isNarrow && "border"));

const getNormalDayCellBorderColorClass = (info: DayCellInfo) => cn(info.isMajor && "border-foreground/20");
const getMutedDayCellBorderColorClass = (info: DayCellInfo) => cn(info.isMajor && "border-foreground/20");
...
const dayRowCommonClasses: CalendarOptions = {
  ...
  listItemEventClass: (info) =>
    cn(
      "mb-px p-px",
      info.isNarrow ? "mx-px rounded-sm" : "mx-1 rounded-md",
      info.isSelected ? "bg-foreground/5" : "hover:bg-foreground/5",
    ),
  listItemEventInnerClass: (info) =>
    cn("flex flex-row items-center justify-between", info.isNarrow ? `py-px ${xxsTextClass}` : "py-0.5 text-xs"),
  listItemEventTimeClass: (info) =>
    cn(info.isNarrow ? "px-px" : "px-0.5", "text-muted-foreground order-1 whitespace-nowrap overflow-hidden shrink-1"),
  ...
  rowEventClass: (info) =>
    cn(info.isStart && (info.isNarrow ? "ms-0.5" : "ms-1"), info.isEnd && (info.isNarrow ? "me-0.5" : "me-1")),
  ...
  rowMoreLinkClass: (info) =>
    cn(
      "mb-px border",
      info.isNarrow ? "mx-px border-primary rounded-sm" : "self-start mx-1 border-transparent rounded-md",
      "hover:bg-foreground/5",
    ),
  rowMoreLinkInnerClass: (info) => (info.isNarrow ? `p-px ${xxsTextClass}` : "p-0.5 text-xs"),
};

export type EventCalendarViewProps = CalendarOptions & Required<Pick<CalendarOptions, "popoverCloseContent">>; // ensure callers define icons
...
export function EventCalendarViews({ views: userViews, ...restOptions }: EventCalendarViewProps) {
  return (
    <FullCalendar
      ...
      eventClass={(info) =>
        cn(
          info.isSelected ? ["outline-3", info.isDragging && "shadow-md"] : "focus-visible:outline-3",
          "outline-ring/50",
        )
      }
      /* Background Event
      ...
      backgroundEventTitleClass={(info) =>
        cn("opacity-50 italic", info.isNarrow ? `p-1 ${xxsTextClass}` : "p-2 text-xs")
      }
      /* Block Event
      ----------------------------------------------------------------------------------------- */
      ...
      rowEventBeforeClass={(info) =>
        cn(info.isStartResizable && [info.isSelected ? rowTouchResizerClass : rowPointerResizerClass, "-start-1"])
      }
      rowEventAfterClass={(info) =>
        cn(info.isEndResizable && [info.isSelected ? rowTouchResizerClass : rowPointerResizerClass, "-end-1"])
      }
      rowEventInnerClass={(info) => cn("flex flex-row items-center", info.isNarrow ? xxsTextClass : "text-xs")}
      rowEventTimeClass={(info) => cn(info.isNarrow ? "ps-0.5" : "ps-1", "font-medium")}
      rowEventTitleClass={(info) => cn(info.isNarrow ? "px-0.5" : "px-1")}
      /* Column Event
      ----------------------------------------------------------------------------------------- */
      ...
      columnEventBeforeClass={(info) =>
        cn(info.isStartResizable && [info.isSelected ? columnTouchResizerClass : columnPointerResizerClass, "-top-1"])
      }
      columnEventAfterClass={(info) =>
        cn(info.isEndResizable && [info.isSelected ? columnTouchResizerClass : columnPointerResizerClass, "-bottom-1"])
      }
      columnEventInnerClass={(info) =>
        cn(
          "flex",
          info.isShort ? "flex-row items-center gap-1 p-1" : ["flex-col", info.isNarrow ? "px-1 py-0.5" : "px-2 py-1"],
          info.isShort || info.isNarrow ? xxsTextClass : "text-xs",
        )
      }
      columnEventTimeClass={(info) => cn(!info.isShort && (info.isNarrow ? "pt-0.5" : "pt-1"))}
      columnEventTitleClass={(info) => cn(!info.isShort && (info.isNarrow ? "py-0.5" : "py-1"), "font-semibold")}
      /* More-Link
      ----------------------------------------------------------------------------------------- */

      moreLinkClass="focus-visible:outline-3 outline-ring/50"
      moreLinkInnerClass="whitespace-nowrap overflow-hidden"
      columnMoreLinkClass={(info) =>
        cn(
      ...
      columnMoreLinkInnerClass={(info) => cn(info.isNarrow ? `p-0.5 ${xxsTextClass}` : "p-1 text-xs")}
      /* Day Header
      ----------------------------------------------------------------------------------------- */

      dayHeaderAlign={(info) => (info.inPopover ? "start" : "center")}
      dayHeaderClass={(info) => cn("justify-center", info.inPopover && "border-b bg-foreground/5")}
        ...
        cn(
          ...
          !info.dayNumberText && !info.inPopover
            ? [
                ...
                info.isNarrow ? "px-1 m-1 text-muted-foreground" : "px-1.5 m-2 font-semibold",
                info.hasNavLink && mutedHoverButtonClass,
              ]
            : info.isToday && info.dayNumberText && !info.inPopover
              ? ["group m-2 outline-none", info.isNarrow ? "h-6" : "h-8"]
              : [
                  "rounded-sm",
                  info.inPopover ? "m-2 px-1 py-0.5" : ["mx-2 h-6 px-1.5", info.isNarrow ? "my-2" : "my-3"],
                  info.hasNavLink && mutedHoverButtonClass,
                ],
        )
      }
      ...
      dayCellClass={(info) =>
        cn("border", (info.isOther || info.isDisabled) && !info.options.businessHours && "bg-foreground/3")
      }
      dayCellTopClass={(info) => cn(info.isNarrow ? "min-h-0.5" : "min-h-1", "flex flex-row")}
      dayCellTopInnerClass={(info) =>
        cn(
          "flex flex-row items-center justify-center whitespace-nowrap",
          info.isNarrow ? `my-px h-5 ${xxsTextClass}` : "my-1 h-6 text-xs",
          info.isToday
            ? [
                "rounded-full font-semibold",
                info.isNarrow ? "ms-px" : "ms-1",
                info.text === info.dayNumberText ? (info.isNarrow ? "w-5" : "w-6") : info.isNarrow ? "px-1" : "px-2",
      ...
      dayLaneInnerClass={(info) => cn(info.isStack ? "m-1" : info.isNarrow ? "mx-px" : "mx-1")}
      slotLaneClass={(info) => cn("border", info.isMinor && "border-dotted")}
      /* List Day
      ----------------------------------------------------------------------------------------- */

      listDaysClass="my-10 mx-auto w-full max-w-218 px-4"
      listDayClass={(info) => cn(!info.isLast && "border-b", "flex flex-row items-start gap-2")}
      ...
      listDayHeaderInnerClass={(info) =>
        cn(
          ...
          !info.level
            ? [
                info.isToday
                  ? ["font-semibold bg-primary text-primary-foreground", info.hasNavLink && "hover:bg-primary/90"]
                  : ["font-medium", info.hasNavLink && "hover:bg-foreground/5"],
              ]
            : ["text-muted-foreground", info.hasNavLink && "hover:bg-foreground/5 hover:text-muted-foreground"],
        )
      }
      ...
      singleMonthClass={(info) =>
        cn(info.multiMonthColumns > 1 && "m-4", info.multiMonthColumns === 1 && !info.isLast && "border-b")
      }
      singleMonthHeaderClass={(info) =>
        cn(info.multiMonthColumns > 1 ? "pb-1" : "py-1.5 bg-background border-b", "items-center")
      }
      singleMonthHeaderInnerClass={(info) =>
        cn("py-1 px-2 rounded-md text-sm font-semibold", info.hasNavLink && "hover:bg-foreground/5")
      }
      /* Misc Table
      ----------------------------------------------------------------------------------------- */
      ...
      views={{
        ...
        multiMonth: {
          ...
          dayHeaderDividerClass: (info) =>
            cn(info.multiMonthColumns === 1 && "border-b border-foreground/20 shadow-sm"),
          dayCellClass: getNormalDayCellBorderColorClass,
          dayCellBottomClass: getShortDayCellBottomClass,
        ...
        timeGrid: {
          ...
          allDayHeaderInnerClass: (info) => cn("m-3 text-muted-foreground", info.isNarrow ? xxsTextClass : "text-xs"),
        ...
        list: {
          ...
          listItemEventTimeClass:
            "shrink-0 w-1/2 max-w-50 whitespace-nowrap overflow-hidden text-ellipsis text-muted-foreground",
          listItemEventTitleClass: (info) =>
            cn("grow min-w-0 font-medium whitespace-nowrap overflow-hidden", info.event.url && "group-hover:underline"),
