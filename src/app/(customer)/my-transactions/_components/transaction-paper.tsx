import { INVOICE_PAPER_HEIGHT, INVOICE_PAPER_WIDTH } from "@/app/(staff)/invoice/_components/data";
import { formatCurrency } from "@/lib/utils";

export interface TransactionPaperLineItem {
  id: string;
  description: string;
  quantity: number;
  unitPrice: number;
...
export interface TransactionPaperProps {
  reference: string;
  issuedDate: string;
  stateLabel: string;
...
export function TransactionPaper({
  reference,
  issuedDate,
  stateLabel,
  items,
    ...
    >
      <header className="flex flex-col gap-10">
        <div className="grid grid-cols-2 items-start gap-14">
          ...
          <h2 className="text-4xl uppercase tracking-widest">Transaction</h2>
        </div>
        ...
        <section className="text-sm">
          ...
          {items.map((item) => (
            ...
            >
              ...
              <span className="text-right">{formatInvoiceCurrency(item.quantity * item.unitPrice)}</span>
            </div>
          ))}
        </section>
